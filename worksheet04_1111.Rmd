...
##### The residuals are randomly scattered around zero which means equal variance assumption is not violated. But, the sample size is apparantely lower. So, a larger sample size might generate different results. 
```{r}
...
for (i in 1:length(groups)){
  ...
  geom_point() +
  annotate(geom="text", x=8.5, y=10, label=paste("R^2== ",round(summary(lm_fit_i)$r.squared, digits=2)), color="red", parse=TRUE) +
  geom_smooth(method = "lm", color = "darkorange") +
