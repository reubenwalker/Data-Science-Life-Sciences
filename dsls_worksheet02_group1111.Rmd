...
```
So does it make sense to do a post-hoc power analysis?
...
So 
```{r}  
power_simulation <- function(i){ # where i = # samples drawn
  control <- rnorm(n=20, mean=0, sd=1)
  treatment <- rnorm(n=20, mean=0.5, sd=1)
  ...
  alpha_ = 0.05
  type_ = "two.sample"
  power <- power.t.test(n=i, delta=delta_, sd=sd_, sig.level=0.05, power=NULL, type="two.sample")$power
